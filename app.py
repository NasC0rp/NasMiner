...
╚═╝  ╚═══╝╚═╝  ╚═╝╚══════╝╚═╝     ╚═╝╚═╝╚═╝  ╚═══╝╚══════╝╚═╝  ╚═╝
{Fore.WHITE}
       Miner Unmineable - Wallets config, worker manuel
"""
...
def start_mining(coin, wallet, worker, low_power=False):
    check_xmrig()
...
def menu():
    ...
    while True:
        ...
        print("[3] Quitter")
        print(Fore.YELLOW + "[4] Mode Faible Puissance (batterie/CPU faible)\n" + Fore.RESET)
        choice = input(Fore.YELLOW + "👉 Ton choix : " + Fore.RESET)
        if choice == "1":
            coin = select_coin(wallets)
            worker = input_worker()
            start_mining(coin, wallets[coin], worker, low_power=False)
            ...
            coin = select_coin(wallets)
            worker = input_worker()
            start_mining(coin, wallets[coin], worker, low_power=True)
            input(Fore.YELLOW + "\nAppuie sur Entrée pour revenir au menu..." + Fore.RESET)
        else:
